...
public class ConnectorFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Connector config = (Connector) PART_TYPE.CONNECTOR.config.get(0);
		...
		entity.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.CONNECTOR.clazz).get(0),
				config.textureScale));
		return entity;
	}
