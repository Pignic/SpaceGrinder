...
public class ContainerFactory extends ShipPartFactory {

	@Override
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Container config = (Container) PART_TYPE.CONTAINER.config.get(0);
		...
		entity.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.CONTAINER.clazz).get(0),
				config.textureScale));
		return entity;
	}
