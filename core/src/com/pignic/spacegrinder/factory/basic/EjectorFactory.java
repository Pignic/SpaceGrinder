...
public class EjectorFactory extends ShipPartFactory {

	@Override
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Ejector config = (Ejector) PART_TYPE.EJECTOR.config.get(0);
		...
		entity.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.EJECTOR.clazz).get(0),
				config.textureScale));
		return entity;
	}
