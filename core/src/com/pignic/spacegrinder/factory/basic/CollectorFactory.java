...
public class CollectorFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Collector config = (Collector) PART_TYPE.COLLECTOR.config.get(0);
		...
		entity.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.COLLECTOR.clazz).get(0),
				config.textureScale));
		return entity;
	}
