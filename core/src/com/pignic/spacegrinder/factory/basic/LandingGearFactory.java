...
public class LandingGearFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final LandingGear config = (LandingGear) PART_TYPE.LANDING_GEAR.config.get(0);
		...
		entity.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.LANDING_GEAR.clazz).get(0),
				config.textureScale));
		return entity;
	}
