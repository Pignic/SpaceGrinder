...
public class ShieldFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Shield config = (Shield) PART_TYPE.SHIELD.config.get(0);
		...
		armor.add(
				new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.SHIELD.clazz).get(0), config.textureScale));
		return armor;
	}
}
