...
public class ArmorFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Armor config = (Armor) PART_TYPE.ARMOR.config.get(0);
		...
		entity.add(
				new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.ARMOR.clazz).get(0), config.textureScale));
		return entity;
	}
