...
public class BeaconFactory extends ShipPartFactory {
	...
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Beacon config = (Beacon) PART_TYPE.BEACON.config.get(0);
		...
		armor.add(
				new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.BEACON.clazz).get(0), config.textureScale));
		return armor;
	}
