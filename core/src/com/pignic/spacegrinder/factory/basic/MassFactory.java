...
public class MassFactory extends ShipPartFactory {

	@Override
	public Entity buildPart(final World world, final Vector2 position, final float angle) {
		final Mass config = (Mass) PART_TYPE.MASS.config.get(0);
		...
		armor.add(new Renderable(AssetManager.shipPartsTextures.get(PART_TYPE.MASS.clazz).get(0), config.textureScale));
		return armor;
	}
